...
public class Demon_ebill_nts_send {
	...
	public void checkAlive(String sUrl){
        ...
        if(sUrl == null || "".equals(sUrl)){
        	sUrl = "http://localhost/ntsSendBatch.do";
        	//OS check
			...
			if(!osName.startsWith("Windows")){
				...
				sUrl = "http://172.16.1.183/ntsSendBatch.do";
			}
        }
...
//            printByInputStream(urlConnection.getInputStream());
	...
	public static void main(String[] args) {
		...
		System.out.println("-------------Start NTS daemon send-------------");
		log.Write(log_flag, prgm_nm, "-------------Start NTS daemon send-------------");

		String sUrl = "";
		sUrl = args.length > 0 ? "".equals(args[0]) ? "" : args[0] : "";

		Demon_ebill_nts_send demon = new Demon_ebill_nts_send();
		demon.checkAlive(sUrl);
		
		log.Write(log_flag, prgm_nm, "-------------End NTS daemon send-------------");
		System.out.println("-------------End NTS daemon send-------------");
	}
}
