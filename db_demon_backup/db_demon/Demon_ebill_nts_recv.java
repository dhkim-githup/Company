...
public class Demon_ebill_nts_recv {
	...
	public void checkAlive(String sUrl){
        ...
        if(sUrl == null || "".equals(sUrl)){
        	sUrl = "http://localhost/ntsRecvBatch.do";
        	//OS check
    		Properties p = System.getProperties();
    		String osName = (String)p.get("os.name");
			if(!osName.startsWith("Windows")){
				//sUrl = "http://esign.happynarae.co.kr/ntsRecvBatch.do";
				//sUrl = "http://121.78.69.199/ntsRecvBatch.do";
				sUrl = "http://172.16.1.183/ntsRecvBatch.do";
			}
        }
...
//            printByInputStream(urlConnection.getInputStream());
	...
	public static void main(String[] args) {
		...
		System.out.println("-------------Start NTS daemon receive-------------");
		log.Write(log_flag, prgm_nm, "-------------Start NTS daemon receive-------------");

		String sUrl = "";
		sUrl = args.length > 0 ? "".equals(args[0]) ? "" : args[0] : "";

		Demon_ebill_nts_recv demon = new Demon_ebill_nts_recv();

		demon.checkAlive(sUrl);
		
		log.Write(log_flag, prgm_nm, "-------------End NTS daemon receive-------------");
		System.out.println("-------------End NTS daemon receive-------------");
	}
}
